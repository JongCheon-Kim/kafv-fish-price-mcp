...
export function registerTradeActivity(server: McpServer) {
  server.registerTool("get_seafood_trade_activity", {
    title: "수산물 경매·거래 조회",
    description: "실시간 경매·온라인 도매·출하 API를 조회한다. 가격 API와 코드체계가 다르므로 가격 품목코드를 거래 API에 억지로 재사용하지 않는다. 현재 v0.1은 날짜 등으로 조회한 실제 응답에서 품목명을 보수적으로 명칭 매칭한다.",
    ...
    try {
      ...
      const raw = await workerGet(endpoint[args.channel], params);
      ...
      return toolResult({ status: rows.length ? "available" : "empty", request: args, source: { endpoint: endpoint[args.channel], matching: "name-only-v0.1" }, result: { rowCount: rows.length, rows: rows.slice(0, 100) } });
    } catch (error: any) {
