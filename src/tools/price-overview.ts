...
export function registerPriceOverview(server: McpServer) {
  server.registerTool("get_seafood_price_overview", {
    ...
    try {
      let code = item_code || "";
      if (!code) {
        const resolved = await resolveEntity("item", item_name);
      ...
      const raw = await workerGet("/api/item-overview", { ctgry_cd: config.categoryCode, item_cd: code, item_nm: item_name });
      ...
      return toolResult({
        ...
        source: { workerVersionExpected: "0.7.2", endpoint: "/api/item-overview" },
        result: raw
      });
    } catch (error: any) {
