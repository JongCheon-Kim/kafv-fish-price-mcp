...
export function registerPriceCompare(server: McpServer, deps: WorkerDeps) {
  server.registerTool("compare_seafood_prices", {
    ...
    try {
      ...
      if (!code) {
        const r = await resolveEntity(deps, "item", args.item_name);
      ...
      if (!date && args.compare_by !== "distribution_stage") {
        const ov = await workerGet(deps, "/api/item-overview", { ctgry_cd: KAFV.categoryCode, item_cd: code, item_nm: args.item_name });
      ...
      for (const target of args.targets) {
        const r = await resolveEntity(deps, entityType, target);
      ...
      for (const target of resolvedTargets) {
        const base: Record<string, string | undefined> = { "ctgry_cd::EQ": KAFV.categoryCode, "item_cd::EQ": code };
